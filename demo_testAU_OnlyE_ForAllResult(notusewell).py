...
def main(args):
    ...
    for dir_name in subdirs:
        ...
        for i in tqdm(range(1, len(testdata)-1), desc=f"Processing {dir_name}"):
            ...
            if args.saveDepth or args.saveKpt or args.saveObj or args.saveMat or args.saveImages or 'albedo' in opdict:
            ...
            # Save albedo image
            if 'albedo' in opdict:
                albedo = opdict['albedo'][0]  # Get first image in batch
                # Convert UV texture to image format [C, H, W] -> [H, W, C] and save
                albedo_img = util.tensor2image(albedo)
                cv2.imwrite(os.path.join(savefolder, name, name + '_albedo.jpg'), albedo_img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--savefolder', default='/media/cine/First/HWPJ2/ProjectResult/DetailOBJDemo/' + name,
                        type=str,
