"""
2026 01 25
copied from: decalib/gatfarec_Video_DetailNew_20260104.py
...
from .utils.config_DetailNew_20260125_onlyE import cfg
...
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        self.n_param_part = model_cfg.n_exp  # Only expression, like OnlyExpressA
        ...
        self.param_dict_OnlyE = {i: model_cfg.get('n_' + i) for i in model_cfg.param_list_OnlyE}    # exp only (OnlyExpressionA style)
    ...
    def decompose_code_part(self, code, num_dict):
        ''' Convert a flattened parameter vector to a dictionary of parameters
        OnlyExpressionA style: code_dict.keys() = ['exp'] only
        (num_dict에 따라 달라질 수 있음)
        '''
        code_dict = {}
    ...
    def encode(self, images_224, use_coarse_grad=True, use_detail=True):   #offer: encode(self, images_224, use_detail = True)
        ...
        # DEBUG: decompose_code 후 codedict 확인
        print(f"[DEBUG encode] codedict keys: {list(codedict.keys())}")
        if 'tex' in codedict:
            print(f"[DEBUG encode] codedict['tex'] shape: {codedict['tex'].shape}, dtype: {codedict['tex'].dtype}, device: {codedict['tex'].device}")
            print(f"[DEBUG encode] codedict['tex'] min/max/mean: {codedict['tex'].min().item():.6f}/{codedict['tex'].max().item():.6f}/{codedict['tex'].mean().item():.6f}")
        if 'light' in codedict:
            print(f"[DEBUG encode] codedict['light'] shape: {codedict['light'].shape}, dtype: {codedict['light'].dtype}, device: {codedict['light'].device}")
            print(f"[DEBUG encode] codedict['light'] min/max/mean: {codedict['light'].min().item():.6f}/{codedict['light'].max().item():.6f}/{codedict['light'].mean().item():.6f}")
        ...
        # DEBUG: 복사 후 codedict_our 확인
        print(f"[DEBUG encode] codedict_our keys after copy: {list(codedict_our.keys())}")
        if 'tex' in codedict_our:
            print(f"[DEBUG encode] codedict_our['tex'] shape: {codedict_our['tex'].shape}, dtype: {codedict_our['tex'].dtype}, device: {codedict_our['tex'].device}")
            print(f"[DEBUG encode] codedict_our['tex'] min/max/mean: {codedict_our['tex'].min().item():.6f}/{codedict_our['tex'].max().item():.6f}/{codedict_our['tex'].mean().item():.6f}")
            print(f"[DEBUG encode] codedict_our['tex'] == codedict['tex']: {torch.equal(codedict_our['tex'], codedict['tex'])}")
        if 'light' in codedict_our:
            print(f"[DEBUG encode] codedict_our['light'] shape: {codedict_our['light'].shape}, dtype: {codedict_our['light'].dtype}, device: {codedict_our['light'].device}")
            print(f"[DEBUG encode] codedict_our['light'] min/max/mean: {codedict_our['light'].min().item():.6f}/{codedict_our['light'].max().item():.6f}/{codedict_our['light'].mean().item():.6f}")
            print(f"[DEBUG encode] codedict_our['light'] == codedict['light']: {torch.equal(codedict_our['light'], codedict['light'])}")
    ...
    def decode(self, codedict, codedict_old, rendering=True, iddict=None, vis_lmk=True, vis_au=False, return_vis=True,
        ...
        # DEBUG: decode에서 codedict 확인
        print(f"[DEBUG decode] codedict keys: {list(codedict.keys())}")
        if 'tex' in codedict:
            print(f"[DEBUG decode] codedict['tex'] shape: {codedict['tex'].shape}, dtype: {codedict['tex'].dtype}, device: {codedict['tex'].device}")
            print(f"[DEBUG decode] codedict['tex'] min/max/mean: {codedict['tex'].min().item():.6f}/{codedict['tex'].max().item():.6f}/{codedict['tex'].mean().item():.6f}")
        if 'light' in codedict:
            print(f"[DEBUG decode] codedict['light'] shape: {codedict['light'].shape}, dtype: {codedict['light'].dtype}, device: {codedict['light'].device}")
            print(f"[DEBUG decode] codedict['light'] min/max/mean: {codedict['light'].min().item():.6f}/{codedict['light'].max().item():.6f}/{codedict['light'].mean().item():.6f}")
        
        # DEBUG: codedict_old 확인 (비교용)
        if 'tex' in codedict_old:
            print(f"[DEBUG decode] codedict_old['tex'] shape: {codedict_old['tex'].shape}, dtype: {codedict_old['tex'].dtype}, device: {codedict_old['tex'].device}")
            print(f"[DEBUG decode] codedict_old['tex'] min/max/mean: {codedict_old['tex'].min().item():.6f}/{codedict_old['tex'].max().item():.6f}/{codedict_old['tex'].mean().item():.6f}")
        if 'light' in codedict_old:
            print(f"[DEBUG decode] codedict_old['light'] shape: {codedict_old['light'].shape}, dtype: {codedict_old['light'].dtype}, device: {codedict_old['light'].device}")
            print(f"[DEBUG decode] codedict_old['light'] min/max/mean: {codedict_old['light'].min().item():.6f}/{codedict_old['light'].max().item():.6f}/{codedict_old['light'].mean().item():.6f}")

        albedo = self.flametex(codedict['tex'])
        albedo_old = self.flametex(codedict_old['tex'])
        
        # DEBUG: albedo 생성 후 확인
        print(f"[DEBUG decode] albedo shape: {albedo.shape}, dtype: {albedo.dtype}, device: {albedo.device}")
        print(f"[DEBUG decode] albedo min/max/mean: {albedo.min().item():.6f}/{albedo.max().item():.6f}/{albedo.mean().item():.6f}")
        print(f"[DEBUG decode] albedo_old min/max/mean: {albedo_old.min().item():.6f}/{albedo_old.max().item():.6f}/{albedo_old.mean().item():.6f}")
        ...
        if rendering:
            # DEBUG: render 호출 전 light 확인
            print(f"[DEBUG decode] Before render - codedict['light'] shape: {codedict['light'].shape}")
            print(f"[DEBUG decode] Before render - codedict['light'] min/max/mean: {codedict['light'].min().item():.6f}/{codedict['light'].max().item():.6f}/{codedict['light'].mean().item():.6f}")
            print(f"[DEBUG decode] Before render - codedict_old['light'] min/max/mean: {codedict_old['light'].min().item():.6f}/{codedict_old['light'].max().item():.6f}/{codedict_old['light'].mean().item():.6f}")
            ...
            # DEBUG: render 후 결과 확인
            print(f"[DEBUG decode] After render - ops['images'] min/max/mean: {ops['images'].min().item():.6f}/{ops['images'].max().item():.6f}/{ops['images'].mean().item():.6f}")
            print(f"[DEBUG decode] After render - ops_old['images'] min/max/mean: {ops_old['images'].min().item():.6f}/{ops_old['images'].max().item():.6f}/{ops_old['images'].mean().item():.6f}")
