"""
2026 01 25
copied from: decalib/trainer_Video_DetailNew_20260103.py
OnlyExpressionA 방식: config import 경로 변경
"""
...
class Trainer(object):
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        if self.train_detail and hasattr(self.mymodel, 'ViTDetail'):
            ...
            # D_detail 모드는 train_decoder 플래그에 따라 결정
            if self.train_decoder:
                self.mymodel.D_detail.train()
            else:
                self.mymodel.D_detail.eval()  # train_decoder=False일 때는 frozen
