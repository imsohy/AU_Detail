"""
2025-12-20 23:00작성.
이전 버전: gatfarec_Video_OnlyExpress_WT_DetailNew.py
번경 이유: 이전 버전에서는 DECA의 2d landmark를 계산하지 안흔다. 이 파일은 그냥 AFWE에서 사용하여, deca와 우리 모델의
랜드마크를 동시에 쓰기 위해 추가로 생성한 파일이다.
"""
import os, sys
import torch
import torchvision
...
from .models.vitVideo_Sequence_Cross_WT_DetailNew import ViTDetailEncoderSeque
...
class DECA(nn.Module):
    ...
    def encode(self, images_224):   #offer: encode(self, images_224, use_detail = True)
        ...
        detailcode_ours = self.ViTDetail(
            ...
            global_feature.to(self.device_detail),
            detail_feature
        )
    ...
    def decode(self, codedict, codedict_old, rendering=True, iddict=None, vis_lmk=True, vis_au=False, return_vis=True,
        ...
        verts_old, landmarks2d_old, landmarks3d_old, mp_landmark_old = self.flame(
            shape_params=codedict_old['shape'], 
            expression_params=codedict_old['exp'],
        ...
        ## projection for old
        landmarks2d_old = util.batch_orth_proj(landmarks2d_old, codedict_old['cam'])[:, :, :2]
