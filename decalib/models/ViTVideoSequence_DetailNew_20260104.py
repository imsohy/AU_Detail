"""
2026 01 04
copied from: decalib/models/ViTVideoSequence_DetailNewBranch.py

과거 기록: vitVideoSequence를 갔다가 복사해서, 디테일에 적용시켜서,
detail transformer를 만들었었다. 
-> vitVideoSequence_DetailNew.py 랑 ViTVideoSequence_DetailNewBranch.py
(래퍼랑 본 트랜스포머)

그러다가 교쉼께서 global feature는 피료업스거 아닌가? 앂어서,
detail transformer에 key/value로 detail feature만 나눠서 넣었다.
-> vitVideoSequence_DetailNewBranch.py
...
class ViTDetailEncoderSeque(nn.Module):
    ...
    def forward(self, au_seq, shape_seq, detail_feature_seq):  #HERE, global nono
        # 상위 모듈에서 (B,T,·) 형태의 시퀀스를 그대로 전달받아 ViT에 입력
        return self.vit(au_seq, shape_seq, detail_feature_seq)
# ------------------------------------------------------------
...
#   - Q: AU 시퀀스 토큰들 (x)
#   - K: mean(Shape parameter) 1토큰 (y_k에서 생성)
#   - V: mean(Detail feature) 1토큰 (y_v에서 생성)
# ------------------------------------------------------------
...
class TransformerDetail(nn.Module):
    ...
    def forward(self, x, y_k, y_v):
        ...
        # y_k : shape parameter (shape=[B,1,dim])
...
class ViT_Detail(nn.Module):
    def __init__(self, *, num_features, frames, dim, depth, heads,
        ...
        self.FC_shape = nn.Linear(100, dim, bias=True)
    ...
    def forward(self, au_seq, shape_seq, detail_feature_seq):
        """
        au_seq            : (T, 27, 512)    # AU 시퀀스 피처 (현재 코어스 입력과 동일 가정)
        shape_seq         : (T, 100)      # Shape parameter 시퀀스 (E_flame의 parameters_224 앞 100차원)
        ...
        y_k = torch.mean(self.FC_shape(shape_seq), dim=0).view(1,1,-1)
