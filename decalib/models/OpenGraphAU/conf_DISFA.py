...
parser.add_argument('--dataset', default='DISFA', type=str, choices=['BP4D','DISFA','hybrid'], help="experiment dataset BP4D / DISFA / hybrid Dataset")

# Param
parser.add_argument('-b','--batch-size', default=1, type=int, metavar='N', help='mini-batch size (default: 128)')
...
# Experiment
parser.add_argument('--exp-name', default="demo", type=str, help="experiment name for saving checkpoints")
# parser.add_argument('--resume', default='/mnt/hdd/EncoderTrainingCode/Code/decalib/models/OpenGraphAU/checkpoints/OpenGprahAU-SwinT_second_stage.pth', type=str, metavar='path', help='path to latest checkpoint (default: none)')
# parser.add_argument('--resume', default='/home/cine/Documents/HJCode/GANE_code/decalib/models/OpenGraphAU/checkpoints/OpenGprahAU-ResNet50_second_stage.pth', type=str, metavar='path', help='path to latest checkpoint (default: none)')
parser.add_argument('--resume', default='/home/cine/Documents/HJCode/AU_sequenceTest/decalib/models/OpenGraphAU/checkpoints/MEFARG_resnet50_DISFA_fold3.pth', type=str, metavar='path', help='path to latest checkpoint (default: none)')
...
parser.add_argument('--stage', default=2, type=int, choices=[1, 2], help='model stage')
...
def get_config():
    ...
    if cfg.dataset == 'BP4D':
        ...
        with open('config/DISFA_config.yaml', 'r') as f:
            datasets_cfg = yaml.full_load(f)
            datasets_cfg = edict(datasets_cfg)

    elif cfg.dataset == 'hybrid':
        with open('/home/cine/Documents/HJCode/GANE_code/decalib/models/OpenGraphAU/config/hybrid_config.yaml', 'r') as f:
            datasets_cfg = yaml.full_load(f)
            datasets_cfg = edict(datasets_cfg)
    else:
