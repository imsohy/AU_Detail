...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout = 0.):
        ...
        self.cross_attention = Cross_Attention(dim, heads=heads, dim_head=dim_head, dropout=dropout)
        self.feeedForward = FeedForward(dim, mlp_dim, dropout = dropout)


    def forward(self, x, y):
        x = self.attention(x) + x
        x = self.cross_attention(x,y) + x
        x = self.feeedForward(x) + x
