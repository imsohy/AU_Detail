...
from .datasets import build_datasets_video_WT_DetailNew as build_datasets
...
class Trainer(object):
    ...
    def load_checkpoint(self):
        ...
        if os.path.exists(self.cfg.pretrained_modelpath):
            checkpoint = torch.load(self.cfg.pretrained_modelpath)  # 한 번만 로드
            for key in model_dict.keys():
                if key in checkpoint.keys():
                    util.copy_state_dict(model_dict[key], checkpoint[key])
            ...
            if 'D_detail' in checkpoint.keys() and hasattr(self.mymodel, 'D_detail_original'):
                ...
                self.mymodel.D_detail_original.eval()  # Ensure it stays in eval mode
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        images_224 = batch['image_224']     #CPU load.
        ...
        #print(f"[DEBUG] batch['mask'] 원본 shape: {batch['mask'].shape}")
        masks = batch['mask'].to(self.device); 
        #print(f"[DEBUG] masks.to(device) 후 shape: {masks.shape}")
        masks = masks.view(-1,images_224.shape[-3], images_224.shape[-2], images_224.shape[-1])
        #print(f"[DEBUG] masks.view() 후 shape: {masks.shape}")


        #-- encoder
        codedict_old, codedict = self.mymodel.encode(images_224) #split image automaticalliy in encode
        # images = images_224
        #self.midframe = cfg.dataset.K //2
        images = images_224[self.middleframe:self.middleframe+1].to(self.device, non_blocking=True)
      ...
      #       rend_au = self.AU_net(opdict['rendered_images'])[1]
        ...
        pose_d = posecode.to(self.mymodel.device_detail)
        exp_d = expcode.to(self.mymodel.device_detail)
        detail_d = detailcode.to(self.mymodel.device_detail)  # ViTDetail 출력 (이미 device_detail인 경우도 안전하게)

        cond_d = torch.cat([pose_d[:, 3:], exp_d, detail_d], dim=1)
        uv_z = self.mymodel.D_detail(cond_d)  # D_detail은 device_detail에서 실행
        uv_z = uv_z.to(self.device)  # render는 self.device(cuda:0)에서 하므로 이동
