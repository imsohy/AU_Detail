...
from .datasets import build_datasets_video_WT as build_datasets
...
class Trainer(object):
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        # images = images_224
        images = images_224[3:4]
        lmk = lmk[3:4]
        lmk_dense = lmk_dense[3:4]
        masks = masks[3:4]
        batch_size = 1
            ...
            for step in tqdm(range(iters_every_epoch), desc=f"Epoch[{epoch+1}/{self.cfg.train.max_epochs}]"):
                ...
                if self.global_step % self.cfg.train.log_steps == 0:
                    ...
                    if self.global_step % self.cfg.train.vis_steps == 0 or step == (iters_every_epoch-1):
                        ...
                        shape_images_full_old, shape_images_face_old = self.mymodel.render.render_shape(opdict['verts_old'][visind], opdict['trans_verts_old'][visind], images=opdict['images'][visind])
                        if self.vis_au:
                            visdict = {
