...
class SelfDataset(Dataset):
    def __init__(self, K,  image_size, mediapipePath = 'mediapipe_landmark_embedding.npz'):
        ...
        self.masksList = self.MEADAll[:int(len(self.MEADAll)*0.7)]
        print(self.MEADAll[int(len(self.MEADAll)*0.7):])
        with open("data30_MEAD.txt",'w') as f:
            f.writelines(x.replace("masks","images") for x in self.MEADAll[int(len(self.MEADAll)*0.7):])
        with open("data30_MEAD2.txt", 'w') as f:
            f.writelines(self.MEADAll[int(len(self.MEADAll) * 0.7):])
