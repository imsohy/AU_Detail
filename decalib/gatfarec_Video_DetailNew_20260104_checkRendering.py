...
from .utils.config_wt_DetailNew_20260103 import cfg
...
class DECA(nn.Module):
    ...
    def decode(self, codedict, codedict_old, rendering=True, iddict=None, vis_lmk=True, vis_au=False, return_vis=True,
        ...
        if rendering:
            # === [Identity Check] Capture inputs before Coarse Rendering ===
            _capture_albedo = albedo.clone()
            _capture_light = codedict['light'].clone()
            _capture_verts = verts.clone()
            ...
            if getattr(self, 'zero_uv_z', False):
                uv_z = uv_z * 0.0
            if iddict is not None:
                cond_id = torch.cat([
                    iddict['pose'][:, 3:], #jaw(3)
            ...
            # === [Identity Check] Verification before Detail Shading ===
            _diff_albedo = torch.max(torch.abs(_capture_albedo - albedo))
            ...
            opdict['albedo'] = albedo
            opdict['albedo_old'] = albedo_old # DECA 알베도 저장
            opdict['uv_coarse_normals'] = self.render.world2uv(ops['normals']).detach()
            opdict['uv_detail_normals'] = uv_detail_normals
            opdict['displacement_map'] = uv_z + self.fixed_uv_dis[None, None, :, :]
            # end OUR detail render

            # === [실험 3] DECA Albedo + 우리 Shading 교차 결합 ===
            if getattr(self, 'use_deca_albedo', False):
                # 1. DECA Albedo + 우리 Coarse Shading
                albedo_old_on_grid = F.grid_sample(albedo_old, ops['grid'], align_corners=False)
                render_deca_alb_coarse = images * (1 - opdict['v_colors']) + (albedo_old_on_grid * ops['shading_images']) * opdict['v_colors']
                opdict['render_deca_alb_coarse'] = render_deca_alb_coarse

                # 2. DECA Albedo + 우리 Detail Shading
                uv_texture_deca_alb = albedo_old * uv_shading
                predicted_detail_deca_alb = F.grid_sample(uv_texture_deca_alb, ops['grid'], align_corners=False)
                render_deca_alb_detail = images * (1 - opdict['v_colors']) + predicted_detail_deca_alb * opdict['v_colors']
                opdict['render_deca_alb_detail'] = render_deca_alb_detail
            ...
            if return_vis:
                ...
                visdict = {
                    ...
                    'render_deca_alb_coarse': opdict.get('render_deca_alb_coarse'),
                    'render_deca_alb_detail': opdict.get('render_deca_alb_detail'),
                    'uv_albedo': albedo,
                    'shape_images': shape_images,
                    'shape_detail_images': shape_detail_images,
                    'uv_normal_coarse': (opdict['uv_coarse_normals'] * 0.5 + 0.5),
                    'uv_normal_detail': (uv_detail_normals * 0.5 + 0.5),
                    'uv_displacement': (uv_z - uv_z.min()) / (uv_z.max() - uv_z.min() + 1e-8),
    ...
    def visualize(self, visdict, size=224, dim=2):
        ...
        If use_deca_albedo is True, creates 5 columns. Otherwise 3.
        '''
        if getattr(self, 'use_deca_albedo', False):
            # 5-column layout for professor's experiment
            row1_keys = ['inputs', 'render_images_coarse_only', 'render_images_with_detail', 'render_deca_alb_coarse', 'render_deca_alb_detail']
            row2_keys = ['blank_image', 'shape_images', 'shape_detail_images', 'blank_image', 'blank_image']
            row3_keys = ['blank_image', 'shape_images_old', 'shape_detail_images_old', 'blank_image', 'blank_image']
            rows_to_show = [row1_keys, row2_keys, row3_keys]
        else:
            # 3-column layout (Default)
            row1_keys = ['inputs', 'render_images_coarse_only', 'render_images_with_detail']
            row2_keys = ['uv_albedo', 'shape_images', 'shape_detail_images']
            row3_keys = ['uv_normal_coarse', 'uv_normal_detail', 'uv_displacement']
            row4_keys = ['blank_image', 'shape_images_old', 'shape_detail_images_old']
            rows_to_show = [row1_keys, row2_keys, row3_keys, row4_keys]
        
        def process_image(key):
            if key not in visdict or visdict[key] is None:
                return None
            img_tensor = visdict[key]
            if img_tensor.shape[1] == 1: # grayscale to RGB
                img_tensor = img_tensor.repeat(1, 3, 1, 1)
            
            _, _, h, w = img_tensor.shape
            new_h = size
            new_w = int(w * size / h)
            return torchvision.utils.make_grid(F.interpolate(img_tensor, [new_h, new_w]).detach().cpu())
        ...
        def make_row(keys):
            ...
            for key in keys:
                ...
                if img is None:
                    # if blank is needed to keep alignment
                    img = torch.zeros((3, size, size))
                images.append(img)
            return torch.cat(images, dim=2) if images else None

        processed_rows = []
        for row_keys in rows_to_show:
            row_img = make_row(row_keys)
            if row_img is not None:
                processed_rows.append(row_img)
        
        if not processed_rows: return None
        
        grid = torch.cat(processed_rows, dim=1) 
