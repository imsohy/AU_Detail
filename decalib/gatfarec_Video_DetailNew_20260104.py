...
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        if os.path.exists(model_path):  # original DECA's path
            # [참고] 이 블록은 보통 실행되지 않음 (pretrained_modelpath가 빈 문자열이기 때문)
            # 하지만 yml에서 pretrained_modelpath를 설정하면 이 블록이 실행됨
            # 이 경우 checkpoint에 ViTDetail, BiViT가 포함되어 있을 수 있음.
            ...
            # [주요 경로] 이 블록이 실제로 실행됨
            # model_path_224는 원본 DECA 모델(deca_model.tar)로, ViTDetail/BiViT가 포함되지 않음
            # 따라서 여기서는 E_flame, E_detail, D_detail만 로드함
            # ViTDetail과 BiViT는 Trainer.load_checkpoint()에서 별도로 로드됨
            # (trainer_Video_DetailNew_20260104.py의 load_checkpoint 메서드 참고)
            ...
            util.copy_state_dict(self.D_detail.state_dict(), checkpoint['D_detail'])
            util.copy_state_dict(self.D_detail_original.state_dict(), checkpoint['D_detail'])  # Load original weights
            # [의도된 동작] ViTDetail과 BiViT는 여기서 로드하지 않음
            # 이유: model_path_224는 원본 DECA 모델로 ViTDetail/BiViT가 없음
            # 대신 Trainer.load_checkpoint()에서 pretrained_modelpath 또는 resume checkpoint로 로드됨
            parameters = self.E_flame.state_dict()
            self.weight = parameters['layers.0.weight']
            self.bias = parameters['layers.0.bias']
