...
class DECA(nn.Module):
    ...
    # @torch.no_grad()
    def decode(self, lmk_gt,codedict,codedict_old, rendering=True, iddict=None, vis_lmk=True, vis_au=False, return_vis=True, use_detail=False, use_gnn=False,
        ...
        if return_vis and render_orig and original_image is not None and tform is not None:
            ...
            landmarks2d_old = transform_points(landmarks2d_old, tform, points_scale, [h, w])
        ...
        if return_vis:
            ...
            visdict = {
                ...
                # 'landmarks2d_label': util.tensor_vis_landmarks(images, landmarks2d),
                # 'landmarks2d_gt': util.tensor_vis_landmarks(images, lmk_gt),
                # 'landmarks2d': util.tensor_vis_landmarks(images, landmarks2d),
                # 'landmarks2d_wGT': util.tensor_vis_landmarks(images, landmarks2d,lmk_gt),
                # 'landmarks2d_wEMOCAGT': util.tensor_vis_landmarks(images, landmarks2d_old,lmk_gt),
                'landmarks2d_wEMOCAGT': util.tensor_vis_landmarks(images, landmarks2d_old,lmk_gt),
                'landmarks2d_wGT': util.tensor_vis_landmarks(images, landmarks2d, lmk_gt),
