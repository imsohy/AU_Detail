...
from gdl.models.Renderer3 import SRenderY
...
class DecaModule(LightningModule):
    ...
    def _encode_flame(self, images):
        if self.mode == DecaMode.COARSE or \
            ...
            deca_code, exp_deca_code, feature = self.deca._encode_flame(images)
        ...
        code_list, original_code = self.deca.decompose_code([deca_code, exp_deca_code])
        ...
        return code_list, original_code,feature
    ...
    def encode(self, images, training=False) -> dict:
        """
        ...
        code, original_code, features = self._encode_flame(images)
        ...
        codedict['shape'] = shapecode
        codedict['tex'] = texcode
        codedict['exp'] = expcode
    ...
    def decode(self, codedict, training=True, render=True, **kwargs) -> dict:
        ...
        if render:
            ops = self.deca.render(verts, trans_verts, albedo, lightcode, background = images)
            codedict['rendered_image'] = images * (1 - ops['v_colors']) + ops['images'] * ops['v_colors']
    ...
    def _visualization_checkpoint(self, verts, trans_verts, ops, uv_detail_normals, additional, batch_idx, stage, prefix,
        ...
        shape_images_full, shape_images = self.deca.render.render_shape(verts, trans_verts, additional['images'])
        if uv_detail_normals is not None:
            detail_normal_images = F.grid_sample(uv_detail_normals.detach(), ops['grid'].detach(),
                                                 align_corners=False)
            shape_i_detailimages_full, shape_detail_images = self.deca.render.render_shape(verts, trans_verts,additional['images'],
                                                           detail_normal_images=detail_normal_images)
        else:
            shape_detail_images = None

        visdict = {}
        visdict['rendered_image'] = additional['rendered_image'][visind]
        ...
        visdict['geometry_coarse_full'] = shape_images_full[visind]
...
class ExpDECA(DECA):
    ...
    def _create_model(self):
        ...
        if self.config.expression_backbone == 'deca_parallel':
            ...
            self.E_expression, _ = SecondHeadResnet(self.E_flame, self.n_exp_param, 'same')
    ...
    def _encode_flame(self, images):
        ...
        deca_code, feature = super()._encode_flame(images)
        exp_deca_code, exp_feature = self.E_expression(images)
        return deca_code, exp_deca_code, exp_feature
# decom!!!
    def decompose_code(self, code):
        deca_code = code[0]
        expdeca_code = code[1]
