# {'Happiness':0, 'Sadness':1, 'Neutral':2, 'Anger':3, 'Surprise':4, 'Disgust':5, 'Fear':6, 'Contempt':7, 'Snxiety':8, 'Helplessness':9, 'Disappointment':10}

from glob import glob
import os

def update(file, old_str, new_str):
    file_data = ""
    with open(file, "r", encoding="utf-8") as f:
        for line in f:
            if old_str in line:
                line = line.replace(old_str,new_str)            
            file_data += line
    with open(file, "w", encoding="utf-8") as f:
        f.write(file_data)

# old_path = "/scratch/chumache/dfer_datasets/dfew/clip_224x224/"
# old_path = "/home/cine/Downloads/DFEW/DFEW-part2/Clip/clip_224x224/clip_224/"
old_path = "masks" #put your data path here"
new_path = "images" #put your data path here"
#put your data path here"
# all_txt_file = glob(os.path.join('DFEW_set_1_test_sp.txt'))
# for txt_file in all_txt_file:
update("data30_MEAD.txt", old_path, new_path)

# old_path = "/data/EECS-IoannisLab/datasets/MAFW/"
# new_path = "/put your data path here"
# all_txt_file = glob(os.path.join('MAFW_*.txt'))
# for txt_file in all_txt_file:
#     update(txt_file, old_path, new_path)

