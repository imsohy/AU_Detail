...
원본 이미지와 모델이 생성한 이미지 간의 픽셀 단위 NME를 계산합니다.
...
- Pixel-by-Pixel NME 계산: 원본 이미지와 생성 이미지 간의 픽셀 단위 오차 계산
"""
# -*- coding: utf-8 -*-
#
...
def extract_generated_image(opdict, visdict, image_type='render_images_with_detail', frame_idx=1):
    """
    opdict 또는 visdict에서 생성된 이미지 추출
    
    Args:
        opdict: DECA decode 결과의 opdict
        visdict: DECA decode 결과의 visdict
        image_type: 'render_images_with_detail', 'render_images_coarse_only', 
                   'shape_detail_images', 'shape_images', 'rendered_images', etc.
...
def main(args):
    ...
    for dir_name in subdirs:
        ...
        if valid_count > 0:
            ...
            print(f"\nPixel NME Results for {dir_name}:")
            ...
            # 결과를 파일로 저장
            result_file = os.path.join(savefolder, 'pixel_nme_results.json')
            ...
            # 텍스트 파일로도 저장
            result_txt_file = os.path.join(savefolder, 'pixel_nme_results.txt')
            with open(result_txt_file, 'w') as f:
                f.write("Pixel-by-Pixel NME (Normalized Mean Error) Calculation Results - AFEW-VA Dataset\n")
                f.write("=" * 60 + "\n")
    ...
    if all_nme_results:
        print(f"\n{'='*60}")
        print("Overall Pixel NME Results Summary - AFEW-VA Dataset")
        print(f"{'='*60}")
        ...
        # 전체 결과 저장
        summary_file = os.path.join(base_savefolder, 'pixel_nme_summary.json')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='DECA: Pixel-by-Pixel NME Calculation for AFEW-VA Dataset')
    ...
    parser.add_argument('-s', '--savefolder', 
                        default='/media/cine/First/HWPJ2/ProjectResult/Demos/PixelNME_AFEW_Evaluation/', 
                        type=str, 
    ...
    parser.add_argument('--image_type', 
                        default='render_images_with_detail', 
                        type=str,
                        choices=['render_images_with_detail', 'render_images_coarse_only', 
                                'shape_detail_images', 'shape_images', 'rendered_images'],
                        help='Type of generated image to compare')
