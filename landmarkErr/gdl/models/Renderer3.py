...
class Pytorch3dRasterizer(nn.Module):
    ...
    def forward(self, vertices, faces, attributes=None,  h=224, w=224):
...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256):
        ...
        self.flame_mask_file = open("/media/cine/First/data/FLAME_masks.pkl", 'rb')
        self.flame_mask = pickle.load(self.flame_mask_file, encoding='latin1')
    def forward(self, vertices, transformed_vertices, albedos, lights=None, h=224, w=224, light_type='point', background =None):
        '''
        -- Texture Rendering
    ...
    def render_colors(self, transformed_vertices, colors, h=224, w=224):
        '''
    ...
    def render_shape(self, vertices, transformed_vertices, colors=None, images=None, detail_normal_images=None,
        ...
        if images is None:
            shape_images_full = shaded_images * alpha_images_full + torch.zeros_like(shaded_images).to(
                vertices.device) * (1 - alpha_images_full)
            shape_images_face = shaded_images * alpha_images_face + torch.zeros_like(shaded_images).to(
                vertices.device) * (1 - alpha_images_face)
        else:
