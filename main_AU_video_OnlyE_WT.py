...
def main(cfg):
    # windowsize 값을 폴더 이름에 붙이기
    win_size = getattr(cfg.dataset, "windowsize", None)
    if win_size is None:
        win_size = "default"
    run_name = f"win{win_size}"
    ...
    from decalib.trainer_Video_OnlyExpress_WT import Trainer
    cfg.rasterizer_type = 'pytorch3d'
    cfg.device = 'cuda'
    mymodel = DECA(cfg)
...
if __name__ == '__main__':
    ...
    cfg = parse_args(cfg_name='configs/release_version/deca_pretrain_video_OnlyE2_WT.yml')
