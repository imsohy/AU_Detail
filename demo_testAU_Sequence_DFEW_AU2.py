...
def main(args):
    ...
    for videopath in allVideos:
            ...
            os.makedirs(os.path.join(savefolder, 'forExp'), exist_ok=True)
            os.makedirs(os.path.join(savefolder, 'forExp_EMOCA'), exist_ok=True)

            # os.makedirs(os.path.splitext(inputpath)[0].replace("originalImages", "croppedImages1_1_2"),exist_ok=True )

            fileName2 = os.path.join(savefolder, "au27", video_name)
            ...
            for i in tqdm(range(len(testdata) )):
                ...
                for kt, xt in enumerate(au_labels_27):
                    ...
                    with open(fileName2 + "_" + xt + "_WE.txt", "a") as f:
                        f.write(str(opdict['au_img'].cpu().numpy().tolist()[kt]) + "\n")
                    with open(fileName2 + "_" + xt + "R_WE.txt", "a") as f:
                ...
                vis_image = deca.visualize({"render_images":visdict['render_images']}, size=224)
                cv2.imwrite(os.path.join(savefolder, 'forExp', name + '.jpg'), vis_image)
                vis_image = deca.visualize({"render_images_old":visdict['render_images_old']}, size=224)
                cv2.imwrite(os.path.join(savefolder, 'forExp_EMOCA', name + '.jpg'), vis_image)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--savefolder',
                        ...
                        default='/media/cine/First/ForPaperResult/TestReult_New0420/AULoss/sequence_pretrain6/DFEW2/name/',
