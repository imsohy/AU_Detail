...
def main(args):
    ...
    for videopath in allVideos:
        # for k in range(len(actors)):
            ...
            os.makedirs(os.path.join(savefolder, 'kpts'), exist_ok=True)
            os.makedirs(os.path.join(savefolder, 'kpts_mat'), exist_ok=True)
            ...
            for i in tqdm(range(1, len(testdata) - 1)):
                ...
                # rend_au = AU_net(opdict['rendered_images'])
                # rend_au_deca = AU_net(opdict['rendered_images_deca'])
                #
                # opdict['au_img'] = (image_au[1] >= 0.5).float()[0][compare1]
                # opdict['au_rend'] = (rend_au[1] >= 0.5).float()[0][compare1]
                # opdict['au_rend_deca'] = (rend_au_deca[1] >= 0.5).float()[0][compare1]
                ...
                # opdict['au_img'] = (image_au[1] >= 0.5).float()[0][compare2]
                # opdict['au_rend'] = (rend_au[1] >= 0.5).float()[0][compare2]
                # opdict['au_rend_deca'] = (rend_au_deca[1] >= 0.5).float()[0][compare2]
                # # resultI, resultR = vis_au(opdict['au_img'] , opdict['au_rend'] )
                # for kt, xt in enumerate(au_labels):
                #     print(opdict['au_img'][kt], opdict['au_rend'][kt])
                ...
                # if args.saveKpt:
                np.savetxt(os.path.join(savefolder, "kpts", name + '_kpt2d.txt'),
                           opdict['landmarks2d'][0].cpu().numpy())
                scio.savemat(os.path.join(savefolder, "kpts_mat", name.split("Video")[-1].split('_')[0]+"_"+ name.split("frame")[-1].split('_')[0]+ '_lm.mat'),
                             {"lmk_68":opdict['landmarks2d'][0].cpu().numpy()*112+112})
